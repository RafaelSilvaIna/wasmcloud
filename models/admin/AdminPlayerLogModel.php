...
final class AdminPlayerLogModel
{
    ...
    public function summary(string $since): array
    {
        $stmt = $this->db->prepare("
            SELECT
                ...
                SUM(CASE WHEN status = 'open' THEN 1 ELSE 0 END) AS open_logs,
                SUM(CASE WHEN is_embedded_browser = 1 THEN 1 ELSE 0 END) AS embedded_browser,
                SUM(CASE WHEN is_vpn_suspected = 1 THEN 1 ELSE 0 END) AS vpn_suspected,
                COUNT(DISTINCT user_id) AS affected_users
            FROM pipocine_player_logs
            WHERE created_at >= ?
        ");
        $stmt->execute([$since]);
    ...
    public function recent(string $since, int $limit = 30): array
    {
        $stmt = $this->db->prepare("
            SELECT
                id, event_id, severity, event_type, stage, status, user_id, profile_id,
                content_id, content_title, content_type, season_number, episode_number, audio,
                error_title, error_message, technical_message, media_type,
                is_embedded_browser, is_vpn_suspected, browser_name, ip_address,
    ...
    public function byStage(string $since): array
    {
        $stmt = $this->db->prepare("
