...
-- Logs operacionais do player de video, com foco em falhas de reproducao.
...
-- Compatibilidade para tabelas criadas antes do campo de titulo.
SET @ppl_has_content_title := (
    SELECT COUNT(*)
...
SET @ppl_add_content_title := IF(
    @ppl_has_content_title = 0,
    'ALTER TABLE pipocine_player_logs ADD COLUMN content_title VARCHAR(255) NULL AFTER content_id',
    'SELECT 1'
);
PREPARE ppl_stmt FROM @ppl_add_content_title;
EXECUTE ppl_stmt;
DEALLOCATE PREPARE ppl_stmt;

SET @ppl_has_content_title_index := (
    SELECT COUNT(*)
    FROM INFORMATION_SCHEMA.STATISTICS
    WHERE TABLE_SCHEMA = DATABASE()
      AND TABLE_NAME = 'pipocine_player_logs'
      AND INDEX_NAME = 'idx_ppl_content_title_created'
);
SET @ppl_add_content_title_index := IF(
    @ppl_has_content_title_index = 0,
    'ALTER TABLE pipocine_player_logs ADD KEY idx_ppl_content_title_created (content_title, created_at)',
    'SELECT 1'
);
PREPARE ppl_stmt FROM @ppl_add_content_title_index;
EXECUTE ppl_stmt;
DEALLOCATE PREPARE ppl_stmt;
